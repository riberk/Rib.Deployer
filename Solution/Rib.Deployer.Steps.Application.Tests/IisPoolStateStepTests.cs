    ...
    public class IisPoolStateStepTests
    {
        ...
        public void ApplyStopTest()
        {
            ...
            using (var s = CreatePool(poolName))
            {
                ...
                var step = new IisPoolStateStep(new IisApplicationSettings("stop pool", poolName, IisApplicationSettings.State.Stop));
                step.Apply();
        ...
        public void ApplyStartTest()
        {
            ...
            using (var pool = CreatePool(poolName))
            {
                ...
                var step = new IisPoolStateStep(new IisApplicationSettings("start pool", poolName, IisApplicationSettings.State.Start));
                step.Apply();
        ...
        public void RollbackStopTest()
        {
            ...
            using (var pool = CreatePool(poolName))
            {
                pool.EnsureStart();
                Assert.AreEqual(ObjectState.Started, pool.State());
                var step = new IisPoolStateStep(new IisApplicationSettings("Rollback pool", poolName, IisApplicationSettings.State.Stop));
                step.Apply();
                Assert.AreEqual(ObjectState.Stopped, pool.State());
                step.Rollback();
                Assert.AreEqual(ObjectState.Started, pool.State());
        ...
        public void RollbackStartTest()
        {
            ...
            using (var pool = CreatePool(poolName))
            {
                pool.EnsureStop();
                Assert.AreEqual(ObjectState.Stopped, pool.State());
                var step = new IisPoolStateStep(new IisApplicationSettings("Rollback pool", poolName, IisApplicationSettings.State.Start));
                step.Apply();
                Assert.AreEqual(ObjectState.Started, pool.State());
                step.Rollback();
                Assert.AreEqual(ObjectState.Stopped, pool.State());
        ...
        private TempPool CreatePool([NotNull] string name)
        {
            ...
            // 15 установлено методом подбора. ѕерестает падать где-то на 7, 15 - на вс€кий случай
            Thread.Sleep(15);
